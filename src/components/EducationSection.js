...
const Education = ({ isDarkMode }) => {
	const educationDetails = [
		...
		{
			institution: "Chate School and Jr.College of Science,Kolhapur",
			duration: "2017-2019",
			degree: "State Board Class XII Aggregate",
		},
		{
			...
			<div
				...
				{educationDetails.map((edu, index) => (
					<div
						...
						{edu.cgpa && (
							<p className="mt-1 text-sm md:text-base">{edu.cgpa}</p>
						)}
						{edu.score && (
							<p className="mt-1 text-sm md:text-base">{edu.score}</p>
						)}
					</div>
				))}
			</div>
